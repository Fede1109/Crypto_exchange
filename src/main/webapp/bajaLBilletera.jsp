...
<body>
	...
	ArrayList<L_billetera> l_billeteras = controladorBD.todasLineasBilleteras();
			...
			<div class="row grid-responsive">
				<div class="column page-heading">
					<div class="large-card">
						<h1>Baja Linea Billetera</h1>
						<p class="text-large">Aquí podra realizar la baja de lineas de billetera</p>
					</div>
					<div class="row grid-responsive">
						<div class="column ">
							<div class="card">
								<div class="card-title">
									<h3>Baja Linea Billetera</h3>
								</div>
								<div class="card-block">
									<form action="operaciones.jsp?tipo=bajalBilletera" method="post"
											...
											<div class="datos">
												<div class="elementosDerecha" style="width: 45%">
													<div class="dropdown" style="display: flex;flex-direction: column;gap: 2rem;">
														<select required id="l_billetera" name="l_billetera" is="ms-dropdown"
															style="width: 400px" data-child-height="400">
															<option value="">Seleccione la linea de billetera que desee
																eliminar</option>
															<%
															Cliente clie = new Cliente();
															for (int i = 0; i < l_billeteras.size(); i++) {
																Criptomoneda cripto = new Criptomoneda();
																cli = controladorBD.dameCliente(l_billeteras.get(i).getId_billetera());
															%>
															<option data-image="images/criptos/<%=l_billeteras.get(i).getId_criptomoneda()%>.png" value="<%=l_billeteras.get(i).getId_billetera()%>-<%=l_billeteras.get(i).getId_criptomoneda()%>">Pertenece a la billetera de <%=cli.getNombre()%> con  <%=l_billeteras.get(i).getUnidades()%> unidades de <%=controladorBD.dameCripto(l_billeteras.get(i).getId_criptomoneda()).getNombre() %> </option>
															<%
															}
															%>
														</select>
														<div class="alert background-warning" style="width: 500px">
															<em class="fa fa-warning"></em>Se borrarán la linea billetera seleccionada
														</div>
